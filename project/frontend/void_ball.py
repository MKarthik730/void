...
PyQt6 Frontend: glassmorphism UI + glowing ball + radial menu + voice + TTS
"""
...
class GlassWidget(QWidget):
    def __init__(self, parent=None, blur_radius: int = 20):
        ...
        self._bg_color = QColor(6, 8, 14, 245)
        self._border_color = QColor(0, 255, 200, 50)
        self.setAttribute(Qt.WidgetAttribute.WA_TranslucentBackground)

    def set_glass_colors(self, bg: QColor, border: QColor):
        self._bg_color = bg
    ...
    def paintEvent(self, e):
        ...
        glow.setBlurRadius(self._blur_radius)
        glow.setColor(QColor(0, 200, 255, 60))
        glow.setOffset(0, 0)
...
class ChatWindow(QWidget):
    ...
    def _build_ui(self):
        ...
        self._container = GlassWidget(self, blur_radius=25)
        self._container.setFixedSize(420, 540)
        self._container.set_glass_colors(
            QColor(6, 12, 20, 235), QColor(0, 255, 200, 60)
        )
        ...
        self._wa_label.setStyleSheet(
            "color: #4a6a64; font-family: 'Courier New'; font-size: 10px;"
        )
        ...
        self._input.setPlaceholderText("Type or press mic to speak...")
    ...
    def _append_message(self, role: str, text: str):
        ...
        self._chat_area.append(
            f'{prefix}<span style="color:#D0D8D6;">{text}</span><br>'
        )
    ...
    def _process_query(self, text: str):
        ...
        def _run():
            res = api_post("/agent/query", {"text": text})
    ...
    def _show_wa_suggestions(self, reply: str):
        def _run():
            prompt = (
                ...
                "Each max 8 words. Casual tone. "
                f"Format: only 3 suggestions separated by |. Context: {reply[:200]}"
            )
            res = api_post("/agent/query", {"text": prompt})
            suggestions_raw = res.get("response", "")
...
class EmailDraftWindow(QWidget):
    ...
    def _build_ui(self):
        ...
        container = GlassWidget(self, blur_radius=25)
        container.setFixedSize(460, 480)
        container.set_glass_colors(QColor(6, 12, 20, 235), QColor(0, 255, 200, 60))
        ...
        copy_btn.clicked.connect(
            lambda: QApplication.clipboard().setText(self._draft_area.toPlainText())
        )
    ...
    def _record_and_draft(self):
        ...
        def _run():
            ...
            instruction = (
                "Draft a professional, well-structured email. "
                "Include Subject, greeting, body, sign-off.\n\n"
                f"Voice note: {text}"
            )
            res = api_post("/agent/query", {"text": instruction})
            draft = res.get("response", res.get("error", "Failed to draft email"))
...
class WhatsAppWindow(QWidget):
    ...
    def _build_ui(self):
        ...
        title.setStyleSheet(
            "color:#25D366; font-family:'Courier New'; font-size:11px; font-weight:bold; letter-spacing:3px;"
        )
        close_btn = QPushButton("✕")
        close_btn.setFixedSize(24, 24)
        close_btn.setStyleSheet(
            "QPushButton{background:transparent;color:#4a6a64;border:1px solid #1a3535;border-radius:12px;font-size:10px;}QPushButton:hover{color:#25D366;border-color:#25D366;}"
        )
        close_btn.clicked.connect(self.hide)
        header_row.addWidget(title)
        header_row.addStretch()
        ...
        for i in range(3):
            ...
            btn.clicked.connect(
                lambda checked, b=btn: self._send_suggestion(b.text().strip())
            )
    ...
    def _scan_and_suggest(self):
        ...
        def _run(b64=b64):
            print("[WA] Calling /vision/whatsapp-suggest...")
            res = api_post("/vision/whatsapp-suggest", {"screenshot_b64": b64})
    ...
    def _apply_suggestions(self, suggestions: list, error: str):
        if error and not suggestions:
            self._status.setText(f"Error: {error[:80]}")
            return
        if suggestions:
            ...
            self._status.setText("Could not generate suggestions.")
    ...
    def run(self):
        ...
        try:
            if action == "suggest":
                ...
                res = api_post("/agent/query", {"text": text})
                result = res.get("response", res.get("error", "No response"))
                ...
                res = api_post(
                    "/vision/analyze", {"screenshot_b64": b64, "action": "summarize"}
                )
                ...
                b64 = screenshot_b64()
                res = api_post("/vision/save-screenshot", {"screenshot_b64": b64})
                result = (
                ...
                self.status.emit("Capturing screen...")
                b64 = screenshot_b64()
                self.status.emit("Analyzing your screen...")
                res = api_post("/vision/explain", {"screenshot_b64": b64})
                ...
                res = api_post(
                    "/vision/analyze", {"screenshot_b64": b64, "action": "translate"}
                )
                result = res.get("result", res.get("error", "No response"))

            elif action == "history":
                self.status.emit("Fetching history...")
                res = api_get("/memory/history?limit=5")
                logs = res.get("history", [])
                if logs:
                    result = "\n\n".join(
                        [
                            f"[{l['action'].upper()}] {(l.get('output') or '')[:100]}"
                            for l in logs
                        ]
                    )
                else:
...
class IconButton(QWidget):
    ...
    iconScale = property(getScale, setScale)
    ...
    def paintEvent(self, e):
        p = QPainter(self)
        p.setRenderHint(QPainter.RenderHint.Antialiasing)
        s = int(ICON_BTN_SIZE * self._scale)
        offset = (ICON_BTN_SIZE - s) // 2
        ...
        if self.hovered:
            for i in range(3, 0, -1):
                ...
                p.drawRoundedRect(
                    offset - i, offset - i, s - 1 + i * 2, s - 1 + i * 2, 7, 7
                )

        bg_alpha = 60 if self.hovered else 20
        p.setBrush(QBrush(QColor(cr, cg, cb, bg_alpha)))
        p.setPen(Qt.PenStyle.NoPen)
        p.drawRoundedRect(offset, offset, s - 1, s - 1, 6, 6)

        pen = QPen(QColor(cr, cg, cb, 200 if self.hovered else 100))
        pen.setWidth(1)
        p.setPen(pen)
        p.setBrush(Qt.BrushStyle.NoBrush)
        p.drawRoundedRect(offset, offset, s - 1, s - 1, 4, 4)

        tick = 6
        p.drawLine(offset, offset, offset + tick, offset)
        p.drawLine(offset, offset, offset, offset + tick)
        p.drawLine(offset + s - 1, offset, offset + s - 1 - tick, offset)
        p.drawLine(offset + s - 1, offset, offset + s - 1, offset + tick)
        p.drawLine(offset, offset + s - 1, offset + tick, offset + s - 1)
        p.drawLine(offset, offset + s - 1, offset, offset + s - 1 - tick)
        p.drawLine(
            offset + s - 1, offset + s - 1, offset + s - 1 - tick, offset + s - 1
        )
        p.drawLine(
            offset + s - 1, offset + s - 1, offset + s - 1, offset + s - 1 - tick
        )

        sym_col = QColor(cr, cg, cb, 255 if self.hovered else 180)
        p.setPen(sym_col)
        p.setFont(QFont("Segoe UI Symbol", 16, QFont.Weight.Normal))
        p.drawText(
            QRectF(offset, offset - 4, s - 1, s),
        ...
        p.drawText(
            QRectF(offset, offset + s - 13, s - 1, 11),
            Qt.AlignmentFlag.AlignCenter,
            self.data.get("label", ""),
        )
...
class ResultPopup(QWidget):
    ...
    def paintEvent(self, e):
        ...
        glow.setColor(QColor(0, 180, 220, 80))
        glow.setOffset(0, 0)
        self.setGraphicsEffect(glow)
        ...
        p.setPen(cp)
        t = 10
        p.drawLine(1, 1, 1 + t, 1)
        p.drawLine(1, 1, 1, 1 + t)
        p.drawLine(w - 1, 1, w - 1 - t, 1)
        p.drawLine(w - 1, 1, w - 1, 1 + t)
        p.drawLine(1, h - 1, 1 + t, h - 1)
        p.drawLine(1, h - 1, 1, h - 1 - t)
        p.drawLine(w - 1, h - 1, w - 1 - t, h - 1)
        p.drawLine(w - 1, h - 1, w - 1, h - 1 - t)
...
class StatusLabel(QWidget):
    ...
    def paintEvent(self, e):
        ...
        glow.setColor(QColor(0, 255, 200, 40))
        glow.setOffset(0, 0)
        self.setGraphicsEffect(glow)
...
class VoidBall(QWidget):
    ...
    def __init__(self):
        ...
        self._status.move(self.center.x() - 130, self.center.y() + BALL_SIZE // 2 + 10)
    ...
    def paintEvent(self, e):
        ...
        for i in range(6, 0, -1):
            ...
            p.setBrush(
                QBrush(QColor(glow_col.red(), glow_col.green(), glow_col.blue(), alpha))
            )
            p.setPen(Qt.PenStyle.NoPen)
        ...
        pen1 = QPen(
            QColor(ring_col.red(), ring_col.green(), ring_col.blue(), pen1_alpha)
        )
        pen1.setWidth(2)
        p.setPen(pen1)
        p.setBrush(Qt.BrushStyle.NoBrush)
        ...
        pen2 = QPen(
            QColor(
                core_col.red(), core_col.green(), core_col.blue(), int(60 + 40 * pulse)
            )
        )
        pen2.setWidth(1)
        p.setPen(pen2)
        ...
        tick_pen = QPen(
            QColor(
                ring_col.red(), ring_col.green(), ring_col.blue(), int(180 + 60 * pulse)
            )
        )
        tick_pen.setWidth(2)
        p.setPen(tick_pen)
        ...
        short_pen = QPen(
            QColor(
                ring_col.red(), ring_col.green(), ring_col.blue(), int(80 + 40 * pulse)
            )
        )
        ...
        core_grad.setColorAt(
            0,
            QColor(
                core_col.red(), core_col.green(), core_col.blue(), int(200 + 55 * pulse)
            ),
        )
        core_grad.setColorAt(
            0.4,
            QColor(
                core_col.red(), core_col.green(), core_col.blue(), int(80 + 40 * pulse)
            ),
        )
        core_grad.setColorAt(
            1, QColor(core_col.red(), core_col.green(), core_col.blue(), 0)
        )
        ...
        if self.thinking:
            ...
            fsize = 8
        p.setPen(txt_col)
        p.setFont(QFont("Courier New", fsize, QFont.Weight.Bold))
        p.drawText(
            QRectF(cx - r, cy - r, r * 2, r * 2), Qt.AlignmentFlag.AlignCenter, label
        )

        dot = (
            QColor(0, 255, 100, 200) if not self.thinking else QColor(255, 180, 0, 200)
        )
...
if __name__ == "__main__":
    ...
    try:
        ...
        keyboard.add_hotkey("alt+space", toggle_void)
        print("Alt+Space hotkey registered")
    except ImportError:
        print("keyboard package not installed - hotkey disabled")
